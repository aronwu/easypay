...
public class EasyPayException extends RuntimeException implements IExceptionToHttpResponse{
    ...
    public static final EasyPayException
        NOT_EXIST_EXCEPTION		= new EasyPayException("对象不存在", NOT_EXIST);
    public static final EasyPayException
        GENERAL_EXCEPTION			= new EasyPayException("服务器内部错误", GENERAL_ERROR);
    public static final EasyPayException
        VALIDATE_EXCEPTION			= new EasyPayException("参数验证错误", VALIDATE_ERROR);
    public static final EasyPayException
        NOT_IMPLEMENTED_EXCEPTION	= new EasyPayException("接口未实现", NOT_IMPLEMENTED);
    public static final EasyPayException
        AUTHEN_EXCEPTION			= new EasyPayException("用户认证失败", AUTHEN_ERROR);
    public static final EasyPayException
        AUTHOR_EXCEPTION			= new EasyPayException("授权验证失败", AUTHOR_ERROR);
    public static final EasyPayException
        NETWORK_EXCEPTION			= new EasyPayException("网络错误", NETWORK_ERROR);
    public static final EasyPayException
        SIGN_VERIFY_EXCEPTION		= new EasyPayException("签名验证失败", SIGN_VERIFY_ERROR);
    public static final EasyPayException
        INPUT_CONTRACT_EXCEPTION = new EasyPayException("输入信息有误", INPUT_CONTRACT_ERROR);
