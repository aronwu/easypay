...
import com.wuswoo.easypay.service.platform.wechat_wap.WechatPaymentWapRequest;
 ...
 */
public class WechatAppPaymentServiceImpl extends AbstractPaymentService {

    private final static Logger logger = LogManager.getLogger(WechatAppPaymentServiceImpl.class);

    @Autowired
    private IPaymentDBService paymentDBService;

    public WechatAppPaymentServiceImpl(PaymentRequest paymentRequest) {
    ...
    public PaymentRequest newPayment(Payment payment) throws EasyPayException {
        ...
        logger.info("Wechat Unifiedorder Spend time: {0}", String.valueOf(timeCostUtil.cost()));
        logger.info("Wechat Unifiedorder Result:{0}", JSONObject.toJSONString(wechatResult));
        //start to generate payment request
        PaymentRequest wechatRequest = new WechatPaymentAppRequest();
        if ("SUCCESS".equalsIgnoreCase(wechatResult.getResult_code())) {
            ...
            wechatRequest.addParam("package","Sign=WXPay");
            wechatRequest.addParam("nonceStr", RandomStringUtils.randomStringByLength(30));
            wechatRequest.addParam("timeStamp", String.valueOf(System.currentTimeMillis()/1000));
            wechatRequest.addParam("prepayid", wechatResult.getPrepay_id());
            wechatRequest.addParam("partnerid", wechatResult.getMch_id());
